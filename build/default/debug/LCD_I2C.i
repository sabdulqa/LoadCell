...
# 1 "./LCD_I2C.h" 1
# 81 "./LCD_I2C.h"
unsigned char LCD_BL_Status = 1;

unsigned char pin_E;
unsigned char pin_RW;
unsigned char pin_RS;
unsigned char pin_D4;
unsigned char pin_D5;
unsigned char pin_D6;
unsigned char pin_D7;
unsigned char pin_BL;
# 101 "./LCD_I2C.h"
...
void LCD_open()
{
    ...
    SSPCON2bits.SEN = 1;
    while ((SSPCON2 & 0x1F) || (SSPSTAT & 0x04));
    while ((SSPCON2 & 0x1F) || (SSPSTAT & 0x04)); SSPBUF=(unsigned char)0x4E; while( SSPSTATbits.BF );
    SSPCON2bits.ACKDT=0; SSPCON2bits.ACKEN=1; while(RCEN && SSPCON2bits.ACKEN);
}
...
void _LCD_write_upper_nibble(unsigned char u)
{
   ...
   pin_E = 0;
   while ((SSPCON2 & 0x1F) || (SSPSTAT & 0x04)); SSPBUF=_LCD_build_byte(); while( SSPSTATbits.BF );;
   pin_E = 1;
   while ((SSPCON2 & 0x1F) || (SSPSTAT & 0x04)); SSPBUF=_LCD_build_byte(); while( SSPSTATbits.BF );;
   pin_E = 0;
   while ((SSPCON2 & 0x1F) || (SSPSTAT & 0x04)); SSPBUF=_LCD_build_byte(); while( SSPSTATbits.BF );;
